...
def playback_dataset(args):
    ...
    obs_video_path = os.path.join(args.video_path, f"{args.dataset.split('/')[-1].split('.hdf5')[0]}_{args.n}_eps_obs.mp4")
    gt_actions_video_path = os.path.join(args.video_path, f"{args.dataset.split('/')[-1].split('.hdf5')[0]}_{args.n}_eps_gt_actions.mp4")
    ...
    obs_video_writer = imageio.get_writer(obs_video_path, fps=20)
    gt_actions_video_writer = imageio.get_writer(gt_actions_video_path, fps=20)

    for ind in range(len(demos)):
        ep = 'demo_50'# ep = demos[ind]
        print("Playing back episode: {}".format(ep))

        playback_trajectory_with_obs(
            traj_grp=f["data/{}".format(ep)], 
            video_writer=obs_video_writer, 
        ...
        playback_trajectory_with_env(
            ...
            render=False, 
            video_writer=gt_actions_video_writer, 
            video_skip=args.video_skip,
            camera_names=args.render_image_names,
            first=args.first,
        )

        # Reset the environment and playback_trajectory_with_policy

    f.close()
    obs_video_writer.close()
    gt_actions_video_writer.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video_path",
        type=str,
        default="./dataset_episodes/test",
        help="folder for saving the videos",
    )
